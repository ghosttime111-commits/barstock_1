...
begin
  ...
  if duplicate_group_count > 0 then
    raise exception using
      errcode = '23505',
      message = format(
        'Найдены дубли категорий по сети, зоне и нормализованному названию: %s групп(ы)',
...
-- Remove unique constraints whose only key is public.categories.name.
...
begin
  for constraint_row in
    ...
    where table_schema.nspname = 'public'
      and table_object.relname = 'categories'
...
begin
  ...
  if target_index_oid is not null and not exists (
    ...
    raise exception
      'Индекс public.categories_network_area_name_unique_idx уже существует, но имеет другую структуру';
  end if;
end $$;

create unique index if not exists categories_network_area_name_unique_idx
